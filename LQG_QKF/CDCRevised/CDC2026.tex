...
\begin{figure}[h]
    ...
    \end{subfigure}
    \vspace*{-6mm}
    \caption{\small Sensor utilization ratio versus target accuracy ratio. (Red) mean and variance by the linearized greedy baseline \cite{c9}. (Blue) Mean and variance by the proposed greedy quadratic model.}
    \label{fig:mainfig}
\end{figure}

To evaluate the supermodularity-ratio analysis in Theorem~2, Fig.~2 compares the empirical supermodularity ratio with the proposed lower bound under two parameter sweeps using Monte Carlo simulations. Here, we fix $R_{\mathrm{ratio}}=0.01$ and keep the other parameters unchanged. We compute the empirical supermodularity ratio $\gamma_h$, the proposed bound from Theorem~2 (orange), and the bound from~\cite{c19} (green). The plotted curves show the median across valid trials, and the shaded regions indicate the empirical variance range. In the left panel, we vary the scale of the linear measurement term over $[10^{-2}, 10]$ while keeping the other parameters fixed. As the linear-term scale increases, the observation model moves farther from the restrictive pure-quadratic setting considered in previous work. %TODO(pass-2, CDCRevised variant): the sentences below describe the ORIGINAL (pre-Woodbury-swap) Fig. 2 and have not been re-verified against the new bound -- do not treat as established until Fig. 2 is regenerated for this variant. The empirical supermodularity ratio remains above the proposed lower bound across the full sweep, confirming that the proposed bound serves as a valid conservative certificate. Unlike the previous bound, the proposed analysis holds regardless of the linear term or the rank of $M^{(i)}$ by construction (it never isolates a per-sensor $c_j$ term), and thus remains applicable to the broader mixed linear-quadratic model.



In the right panel, we evaluate the proposed lower bound under a sweep of the measurement-noise scale. We vary the measurement-noise scale over $[10^{0},10^{4}]$ using a fixed nonzero linear-term scale $10$, multiplicated with randomly generated base $C_0$ and the same remaining parameters. %TODO(pass-2, CDCRevised variant): the tightness/competitiveness claims below are UNVERIFIED for the Woodbury bound and must be re-checked against regenerated data before this paragraph is finalized -- see Timeline.md. The empirical supermodularity ratio stays above both theoretical curves across the full sweep, again confirming that the proposed bound is a valid conservative certificate. Moreover, the proposed bound is slightly tighter than, or nearly identical to, the baseline bound over the tested noise range. This suggests that the broader applicability of the proposed analysis does not come at the expense of weaker performance under noise variation, and that the proposed bound remains competitive with the previous result.
