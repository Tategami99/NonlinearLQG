...
2026-09-09 (kept as its own script rather than folded into `fig2_empirical_coverage.py`, since it's still
one candidate among possible others, not a finalized choice).
...
fig.suptitle('Figure 2 candidate: efficiency, side by side with independent y-axis scales', y=0.99, fontsize=14.5)
...
fig.savefig(perf_dir + 'fig2_candidate1_efficiency_side_by_side.png', dpi=200)
plt.close(fig)
print(f"\nSaved {perf_dir}fig2_candidate1_efficiency_side_by_side.png")
